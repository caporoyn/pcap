...
printf("The source MAC address is : <%s>\n ",ether_ntoa(&smac));
printf("The destination MAC address is : <%s>\n\n", ether_ntoa(&dmac));
			if(ntohs(packetHeader->ether_type) == ETHERTYPE_IP){
				printf("The type of IP Protocol is : ( ");
				...
				daddr = *((int*)ipAddrPtr);
				inet_ntop(AF_INET, ipAddrPtr, ipAddrStr, INET_ADDRSTRLEN);
				printf("Ths destination IP address is : <%s>\n\n", ipAddrStr);
				...
				if(*proto == IPPROTO_TCP || *proto == IPPROTO_UDP){
					...
					printf("The source port Number is : [%d]\n", ntohs(*((short*)(packet + 14 + ipHeaderLen))));		// port number is 16-bits wide, so we cast the pointer to a short pointer
					printf("The destination port Number is :[%d]\n", ntohs(*((short*)(packet + 14 + ipHeaderLen + 2))));		// dest port number is next to source port number
				}
			}
	...
	printf("The number of IP packets is : \n");
	int i;
	for(i = 0; i < count; i++){
		char addrStr[64];
